...
public class RawDepthCodelabActivity extends AppCompatActivity implements GLSurfaceView.Renderer {
  ...
  public float floorHeight = Float.MAX_VALUE;
  private boolean hasTrackingPlane() {
    boolean return_val = false;
    for (Plane plane : session.getAllTrackables(Plane.class)) {
      //test
      // Check if this plane is horizontal and facing upwards
      return_val = true;
  ...
  public void onDrawFrame(GL10 gl) {
    ...
    boolean isTrackingPlane = hasTrackingPlane();
    Log.d(TAG, "isTrackingPlane: " + isTrackingPlane);
    ...
    try {
      ...
      FloatBuffer points = DepthData.create(frame, session.createAnchor(camera.getPose()), floorHeight);
      if (points == null) {
        return;
      }
